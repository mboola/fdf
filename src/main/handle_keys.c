...
static int	check_angles(int keys, t_ctrl_prgrm *data)
{
	...
	if (keys == XK_x)
		update_value(&(shape->angle_x), 0.1, data->reverse);
	else if (keys == XK_c)
		update_value(&(shape->angle_y), 0.1, data->reverse);
	else if (keys == XK_z)
		update_value(&(shape->angle_z), 0.1, data->reverse);
	else
		return (0);
	data->rasterize = 1;
	return (1);
...
static int	check_transformation(int keys, t_ctrl_prgrm *data)
{
	...
	{
		update_value(&(shape->scale[0]), 1, data->reverse);
		update_value(&(shape->scale[1]), 1, data->reverse);
	}
	else
		return (0);
	data->rasterize = 1;
